"""Wind speed segmentation using equation-fitting with natural boundaries.

Same walk-forward algorithm as segmentation_pressure.py.

Equation types (synoptic-scale, no solar harmonic):
  0 - exponential: x(t) = L + A·e^(−λt)  — wind decay after frontal passage
  1 - linear:      x(t) = L + c·t         — steady increase/decrease
  2 - constant:    x(t) = L               — stable flow regime

Boundary = point where current equation prediction exceeds tolerance.
Tolerance: 2 knots absolute.
...
def _predict_value(fit: WindspeedFit, t: float) -> float:
...
def _fit_constant(t: np.ndarray, y: np.ndarray) -> WindspeedFit:
    L   = float(np.mean(y))
    rms = float(np.sqrt(np.mean((y - L) ** 2)))
    return WindspeedFit(EQ_CONSTANT, L=L, rms=rms, n_points=len(y))


def _fit_linear(t: np.ndarray, y: np.ndarray) -> WindspeedFit:
    ...
    return WindspeedFit(EQ_LINEAR, L=L, c=c, rms=rms, n_points=len(y))


def _fit_exponential(t: np.ndarray, y: np.ndarray) -> WindspeedFit:
    x0 = float(y[0])
    ...
    try:
        ...
        return WindspeedFit(EQ_EXPONENTIAL, L=L, lam=lam, A=A, rms=rms, n_points=len(y))
    except Exception:
...
def _best_fit(t: np.ndarray, y: np.ndarray) -> WindspeedFit:
    best: WindspeedFit | None = None
...
def segment_windspeed(
    series: np.ndarray,
    timestamps: pd.DatetimeIndex,
    tolerance: float = TOLERANCE_KT,
) -> tuple[list[WindspeedSegment], dict[str, int]]:
    n = len(series)
    if n == 0:
        return [], {}

    t0_wall = time.perf_counter()
    logger.info("windspeed_segmentation_start n=%d tolerance=%.1f kt", n, tolerance)

    segments:  list[WindspeedSegment] = []
    ...
    while seg_start < n:
        ...
        logger.debug("segment id=%d eq=%s duration_h=%d start=%s rms=%.3f kt",
    ...
    logger.info("windspeed_segmentation_complete segments=%d elapsed_s=%.1f",
...
def build_report(segments: list[WindspeedSegment], eq_counts: dict[str, int]) -> dict:
    total      = len(segments)
    ...
    for eq in TRY_ORDER:
        ...
        by_type[eq] = {
            ...
            "mean_rms_kt":      round(float(np.mean([s.fit.rms for s in segs_of_type])), 3),
        }
    ...
    return {
        ...
        "overall_mean_rms_kt": round(float(np.mean(rms_values)), 3),
