"""Pressure segment sequence forecaster (two-pass, same architecture as temperature).

Pass 1 removed — K computed from historical data.
Model: history → K_MAX segment ODE vectors in one forward pass.
K = average number of pressure segments covering T hours on this day-of-year
...
from weather_patterns.pattern.segmentation_pressure import (
    TRY_ORDER, PressureSegment, PressureFit, _predict_value,
)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("artifacts/pressure_two_pass.log", encoding="utf-8"),
    ],
)
...
FULL_ODE_DIM  = 8    # ODE_FEAT_DIM + duration/24
TIME_FEAT_DIM = 4    # sin/cos day-of-year, sin/cos hour-of-day
FEAT_DIM      = FULL_ODE_DIM + TIME_FEAT_DIM   # encoder input: 12
...
def _global_scales(segments: list) -> dict:
    ...
    lams = [abs(s.fit.lam) for s in segments if abs(s.fit.lam) > 1e-6]
    return {
        ...
        "lam": float(np.percentile(lams, 90)) if lams else 1.0,
    }
...
def segment_to_ode_vector(seg, scales: dict) -> np.ndarray:
    v = np.zeros(FULL_ODE_DIM, dtype=np.float32)
    v[EQ_IDX[seg.fit.eq_type]] = 1.0          # [0..2] type one-hot
    v[3] = seg.fit.L   / scales["L"]
    v[4] = seg.fit.c   / scales["c"]
    v[5] = seg.fit.lam / scales["lam"]
    v[6] = seg.fit.A   / scales["A"]
    v[7] = seg.duration_hours / 24.0           # duration (normalised)
    return v
...
def vector_to_fit(v: np.ndarray, scales: dict) -> tuple:
    eq_type = TRY_ORDER[int(np.argmax(v[:3]))]
    L   = float(v[3]) * scales["L"]
    c   = float(v[4]) * scales["c"]
    lam = float(max(v[5], 0.0)) * scales["lam"]
    A   = float(v[6]) * scales["A"]
    dur = max(1, int(round(float(v[7]) * 24)))
    return PressureFit(eq_type=eq_type, L=L, c=c, lam=lam, A=A), dur
...
def compute_K_from_history(
    ...
    if not k_observations:
        return max(1, round(T_hours / 40.0))
    return max(1, min(K_MAX, round(float(np.mean(k_observations)))))
...
class SegmentForecaster(nn.Module):
    def __init__(self):
        ...
        self.head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(D_MODEL * 2, K_MAX * FULL_ODE_DIM),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.head(h).view(-1, K_MAX, FULL_ODE_DIM)


# ── Dataset ────────────────────────────────────────────────────────────────────

def build_dataset(ode_vectors: np.ndarray) -> tuple:
    N = len(ode_vectors)
    seg_targets = np.zeros((N, K_MAX, FULL_ODE_DIM), dtype=np.float32)
...
def train(model, full_vectors, seg_targets, seg_masks, device) -> list[float]:
    model.to(device)
...
def forecast(model, full_vectors, T_hours, query_date, segments, scales, device):
    ...
    with torch.no_grad():
        X = torch.tensor(full_vectors, dtype=torch.float32).unsqueeze(0).to(device)
        segs_pred = model(X)
        segs_np   = segs_pred[-1, :K].cpu().numpy()
...
def decode_to_series(predicted, x0, start_time, n_hours):
    timestamps, values = [], []
...
def main():
    ...
    logger.info("Scales: %s", {k: f"{v:.4f}" for k, v in scales.items()})
    ...
    for i, (fit, dur) in enumerate(predicted):
        logger.info("  seg %d: eq=%s  dur=%dh  L=%.1f  c=%.4f  A=%.6f  lam=%.4f",
                    i + 1, fit.eq_type, dur, fit.L, fit.c, fit.A, fit.lam)

    pred_24h = decode_to_series(predicted, x0, feb1, 24)

    df = pd.read_csv("hly4935_subset.csv", skiprows=23)
    df["date"] = pd.to_datetime(df["date"], dayfirst=True)
    actual = df.set_index("date")["msl"]

    errors = []
    logger.info("%-22s  %8s  %8s  %6s", "timestamp", "actual", "pred", "error")
    for ts in pred_24h.index:
        act  = float(actual.get(ts, float("nan")))
        pred = float(pred_24h.loc[ts])
        err  = act - pred
        errors.append(err)
        logger.info("  %s  %8.1f  %8.1f  %+.2f", ts, act, pred, err)
    ...
    logger.info("T=24h  MAE=%.3f hPa  RMSE=%.3f hPa", mae_24, rmse_24)
    ...
    if len(common):
        ...
        logger.info("T=168h  K=%d  points=%d  MAE=%.3f hPa  RMSE=%.3f hPa",
                    K_w, len(common),
                    float(np.mean(np.abs(errs_w))),
                    float(np.sqrt(np.mean(errs_w ** 2))))

    result = {
        "n_segments":  len(segments),
        "final_loss":  round(float(losses[-1]), 4),
        "forecast_24h": {"K": K, "mae_hpa": round(mae_24, 3), "rmse_hpa": round(rmse_24, 3)},
    }
    Path("artifacts/pressure_two_pass_report.json").write_text(
        json.dumps(result, indent=2), encoding="utf-8"
    )
    logger.info("Done. MAE(24h)=%.3f hPa", mae_24)
